...
# load the model
model = load_model('model/best_accuracy.model')

# webcam feed
cap = cv2.VideoCapture(0)

# checking webcam accessibilty
if cap.isOpened():
    ret, frame = cap.read()
else:
    ret = False
...
while ret:
    ...
    # followed by Hough lines probablistic transform to highlight the straight line proposals to enhance the countuors

    adapThre = cv2.adaptiveThreshold(blur, 255, 1, 1, 11, 5)
    lines = cv2.HoughLinesP(adapThre, 1, np.pi / 180, 100, minLineLength=100, maxLineGap=10)
    img_with_lines = adapThre.copy()

    # overcoming the case when no line is able to find
    try:
        for x1, y1, x2, y2 in lines[:, 0, :]:
            cv2.line(img_with_lines, (x1, y1), (x2, y2), (255, 255, 255), 2)
    except:
        pass

    # Find the proposals sudoku contour
    cnts,heiarchy = cv2.findContours(img_with_lines.copy(), cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    # Taking 5 largest contours for processing asuuming sudoku lies in them
    ...
    for (i, c) in enumerate(cnts):
        ...
        if len(approx) == 4:
            ...
            if w/float(h) >= 1.2 or w/float(h) <= 0.8:
                continue
            ...
            sudoku = four_point_transform(img_with_lines, mainCnt.reshape(4, 2))
            ...
            horizontal,vertical = np.copy(sudoku.copy())
            vertical = np.copy(sudoku.copy())
            ...
            num = cv2.bitwise_xor(sudoku.copy(), grid)
            ...
            # checking distace between first point and vertically downward point
            #True case
            if (full_coords[0][0]) ** 2 + (full_coords[0][1]) ** 2 < (full_coords[1][0]) ** 2 + (
            full_coords[1][1]) ** 2:
            ...
            for r in range(0, num.shape[0] - windowsize_r, windowsize_r):
                for c in range(0, num.shape[1] - windowsize_c, windowsize_c):
                    ...
                    proposals,heiarchy = cv2.findContours(window, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
                    ...
                    if len(proposals) > 0:
                        ...
                        approx_shape = cv2.approxPolyDP(digit, 0.04 * perimeter, True)
