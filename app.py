...
from extraction.digit_extraction import extractDigits
from extraction.processing import processImage
from extraction.grid_extraction import extractGrid
from classification.recognise_digits import predictDigits
...
def get_np_array_row_major(inp: str, rows: int, cols: int) :
...
def get_matplotlib_figure(orig_img_array: np.ndarray, extracted_digits: np.ndarray, recognised_digits: np.ndarray, solved_grid: np.ndarray):
...
def operate_on_image(img_array):
  ...
  if status == False:
    ...
    recognised_digits_str = predictDigits(extracted_digits)
    ...
    solver = sudoku.suitable(recognised_digits_str)

    if not solver or len(solver)!=81:
        st.write("The puzzle extracted is invalid")
        figure = get_matplotlib_figure(img_array, extracted_digits, recognised_digits, None)
    else:
        solved_grid = get_np_array_row_major(solver, 9, 9)
...
def read_example_image(name):
  img = cv2.imread("example/" +  name + ".jpg")
  return img
...
Pytorch : For implementing ML models for recognizing digits extracted from the image  


Author: [Akshay Gupta](https://github.com/akshay-gupta)   
You can find the code [here](https://github.com/akshay-gupta123/Sudoku-Solver)  
